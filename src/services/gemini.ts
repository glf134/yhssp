...
const HAZARD_SCHEMA = {
  ...
  required: ["name", "category", "riskLevel", "suggestion", "regulations", "references"]
};
...
export async function identifyHazardFromImage(base64Image: string): Promise<HazardResult> {
  const response = await ai.models.generateContent({
    ...
    contents: [
      {
        parts: [
          { text: "你是一个工业安全专家。请分析这张图片中的安全隐患。请详细说明存在的隐患、处置建议、违反了什么规定章程，以及引用了哪些文档。请以JSON格式返回结果。" },
          {
            inlineData: {
              mimeType: "image/jpeg",
...
export async function identifyHazardFromText(text: string): Promise<HazardResult> {
  const response = await ai.models.generateContent({
    ...
    contents: [
      {
        parts: [
          { text: `你是一个工业安全专家。请分析以下描述中的安全隐患： "${text}"。请详细说明存在的隐患、处置建议、违反了什么规定章程，以及引用了哪些文档。请以JSON格式返回结果。` }
        ]
      }
    ],
    config: {
